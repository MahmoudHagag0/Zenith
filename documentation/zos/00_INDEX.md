...
| M6-001 | M6-001_VISUAL_IDENTITY_CONSTITUTION.md | Milestone M6 (Visual Identity Package) — product personality, emotional goals, and per-domain visual philosophy (background/color/typography/layout/card/chart/table/motion/icon/AI-presentation/accessibility/decision-first); the Zenith Identity Test every screen must pass. States the scope resolution: structure/psychology (D1, M4, M5) stay locked, D2's specific values are deliberately superseded. |
| M6-002 | M6-002_VISUAL_LANGUAGE.md | Milestone M6 (Visual Identity Package) — the Constitution translated into direction-agnostic visual rules (background/surface/elevation/shadow/radius/spacing/density/whitespace/rhythm/hierarchy/icon/illustration/chart/table/animation/micro-interaction language). |
| M6-003 | M6-003_VISUAL_DIRECTIONS.md | Milestone M6 (Visual Identity Package) — three complete visual directions (Executive Intelligence, Premium Trading Terminal, Modern Intelligence), each scored on 8 criteria; Executive Intelligence selected, the other two formally rejected. |
| M6-004 | M6-004_OFFICIAL_DESIGN_SYSTEM.md | Milestone M6 (Visual Identity Package) — concrete values for the selected direction (colors, typography, radius, elevation/borders, spacing/container widths, motion, Skeleton, hover/focus/selection, Button/Input/Table/Scrollbar spec), including self-review WCAG AA corrections. Implemented in `packages/design-tokens`/`packages/ui` and applied to the Dashboard. |
