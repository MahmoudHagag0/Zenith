...
-   S1-012's Confluence Engine implements only `EqualWeightStrategy` — real differential (data-driven) weighting is intentionally deferred until historical validation data exists, and requires its own superseding/additional Decision Log entry (the `ConfluenceWeightStrategy` interface exists precisely so this needs no Provider or Confluence Engine contract change). The "expected vs. unexpected disagreement" Methodology Conflict Matrix classification (`22_ANALYSIS_ENGINE_ARCHITECTURE.md`'s own Additional Findings) is also deferred — V1 reports raw, correctly-attributed disagreement only, since no Provider yet self-declares a conflict-expectation tag. Methodology-family-aware grouping is proven correct only via a dedicated fixture test — no two of the three currently-registered Providers share a `methodologyFamily`, so this mechanism remains unexercised by real Provider data until a fourth Provider triggers it. Trace Store persistence remains deferred — Confluence is still internal/composable only, computed live per request, with full per-Provider traceability referenced by ID (recoverable from the same request/response's own `AnalysisProviderResult[]`), never embedded. See `documentation/zos/sprints/S1-012_SPRINT_BRIEF.md`, Missing Decisions section, and DEC-2026-016.
