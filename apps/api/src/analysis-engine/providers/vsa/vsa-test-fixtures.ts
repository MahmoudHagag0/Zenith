...
import type { Swing, SwingDetectionResult, TrendDirection } from '../../swing-detection/swing-detection.types';
import type { RegimeContextResult, VolatilityState } from '../../regime-context/regime-context.types';

/**
 * Shared fixture builders for every `vsa/*.spec.ts` file. Not itself a
...
/** Builds `count` uniform, unremarkable bars (average volume, average spread) preceding a scenario -- a neutral baseline the scenario's own bar stands out against. */
export function buildBaselineBars(count: number, startOffset: number, baseVolume = 1000): MarketSeriesPoint[] {
  const bars: MarketSeriesPoint[] = [];
  for (let i = 0; i < count; i++) {
    bars.push(candle(startOffset + i, { open: 100, high: 101, low: 99, close: 100.5 }, baseVolume));
  }
  return bars;
}
...
export function buildSwingResult(swings: readonly Swing[] = []): SwingDetectionResult {
  return {
    sensitivity: 3,
    swings,
    structureEvents: [],
...
export function buildRegimeResult(trendDirection: TrendDirection = 'UNKNOWN', volatilityState: VolatilityState = 'LOW'): RegimeContextResult {
  return {
    trendState: 'RANGING',
    trendDirection,
    volatilityState,
